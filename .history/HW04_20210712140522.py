...
def groceryShopping(groceryList, priceList, budget):
    ...
    for i in range(len(groceryList):
        if priceList[i] + totalCost <= budget:
            totalCost += priceList
            finalItems += (groceryList[i], priceList[i])
...
groceryList = ["chips", "bagels", "coffee", "lettuce", "milk", "steak"]>>> priceList = [3.50, 6.50, 3.75, 3.00, 4.40, 16.99] >>> budget = 14.50>>> groceryShopping(groceryList, priceList, budget) 
