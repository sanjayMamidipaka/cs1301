...
def monthlyAllowance(allowance, savingsPercentage):
    amountLeftOver = allowance * ((100-savingsPercentage)/100) #calculates the amount left after savings
    amountLeftOver = amountLeftOver - 5.50 - 3.99 #subtracts the netlfix and spotify subscriptions from the amount left over
    amountLeftOverRounded = round(amountLeftOver, 2) #rounds the leftovers to 2 digits
    print('You have ${} left after savings and fees.'.format(amountLeftOverRounded)) #prints my results
